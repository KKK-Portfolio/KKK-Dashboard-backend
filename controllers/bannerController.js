...
exports.createImage = async (req, res) => {
  try {
    ...
    const newImage = new Banner({ filename, filepath });
...
exports.updateImageById = async (req, res) => {
  try {
    ...
    const existingImage = await Banner.findById(req.params.id);
    if (!existingImage) {
      return res.status(404).json({ message: "Image not found" });
    }
    ...
    const { filename, path: filepath } = req.file;
    const updatedImage = await Banner.findByIdAndUpdate(
      req.params.id,
      { filename, filepath },
      { new: true }
    );
...
exports.deleteImageById = async (req, res) => {
  try {
    const image = await Banner.findByIdAndDelete(req.params.id);
    if (!image) {
      return res.status(404).json({ message: "Image not found" });
    }
